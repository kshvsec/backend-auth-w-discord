...
color=0x00ffe6
token = ""
kshv = commands.Bot(command_prefix=".", intents=discord.Intents.all(), help_command=None)
...
async def resetbackend(ctx):
    if ctx.author.id in owner:
        ...
        file.close()
        file2.close()
        await ctx.reply("> Backend has been reset, Thank you for chosing our service <3")
    else:
